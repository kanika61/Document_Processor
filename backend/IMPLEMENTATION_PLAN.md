...
- Status updates to the UI: the detail page currently polls every 3s while a document is in flight. At that scale, replace polling with Server-Sent Events: the worker publishes each status change (e.g. via the queue or Redis pub/sub) and a streaming endpoint (Spring `SseEmitter`) pushes it to subscribed browsers, so idle viewers cost nothing. The existing `GET /documents/{id}` and `/history` endpoints stay for the initial load.
